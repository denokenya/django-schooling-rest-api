...
def NamedTempFile(
    ...
    if sys.version_info < (3, 5):  # covers also python 2
        ...
        if prefix is None:
            prefix = "tmp"
    (fd, fname) = tempfile.mkstemp(suffix=suffix, prefix=prefix, dir=dirname, text=text)
    f = io.open(
        fd,
        mode=mode,
        buffering=buffering,
...
@contextlib.contextmanager
def TempFileContents(dirname, contents, encoding="utf-8", newline="", suffix=None):
    # Note: NamedTempFile properly handles unicode encoding when using mode='w'
    with NamedTempFile(
        dirname=dirname, mode="w", encoding=encoding, newline=newline, suffix=suffix
    ) as (f, fname):
        f.write(contents)
        f.flush()
        yield fname
