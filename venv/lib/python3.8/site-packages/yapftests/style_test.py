...
class UtilsTest(unittest.TestCase):
    def testContinuationAlignStyleStringConverter(self):
        for cont_align_space in ("", "space", '"space"', "'space'"):
            self.assertEqual(
                style._ContinuationAlignStyleStringConverter(cont_align_space), "SPACE"
            )
        for cont_align_fixed in ("fixed", '"fixed"', "'fixed'"):
            self.assertEqual(
                style._ContinuationAlignStyleStringConverter(cont_align_fixed), "FIXED"
            )
...
class StyleFromFileTest(unittest.TestCase):
    ...
    def testPyprojectTomlNoYapfSection(self):
        ...
        _ = open(filepath, "w")
        with self.assertRaisesRegexp(style.StyleConfigError, "Unable to find section"):
            style.CreateStyleFromConfig(filepath)
