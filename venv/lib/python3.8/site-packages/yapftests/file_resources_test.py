...
class GetDefaultStyleForDirTest(unittest.TestCase):
    ...
    def test_local_style_at_root(self):
        ...
        with _exists_mocked_in_module(
            file_resources.os.path, mock_exists_implementation
        ):
...
class GetCommandLineFilesTest(unittest.TestCase):
            ...
            self.assertEqual(
                ...
                sorted(
                    [
                        os.path.join(
                            os.path.relpath(self.test_tmpdir),
                            os.path.basename(tdir2),
                            "testfile2.py",
                        ),
                        os.path.join(
                            os.path.relpath(self.test_tmpdir),
                            os.path.basename(tdir3),
                            "testfile3.py",
                        ),
                    ]
                ),
    ...
    def test_find_with_excluded_dirs(self):
        ...
        found = sorted(
            file_resources.GetCommandLineFiles(
                ["."], recursive=True, exclude=["test1", "test3"]
            )
        )
