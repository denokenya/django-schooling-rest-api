...
def print_provider(doc, provider, formatters, excludes=None, output=None):
    ...
    print(file=output)
    print("### {}".format(doc.get_provider_name(provider)), file=output)
    print(file=output)
    ...
    for signature, example in formatters.items():
        ...
        try:
            ...
            raise Exception('error on "{}" with value "{}"'.format(signature, example))
        ...
        for line in lines:
            for i in range(0, (len(line) // remains) + 1):
                print(
                    "\t{fake:<{margin}}{separator} {example}".format(
                        fake=signature,
                        separator=separator,
    ...
    if provider_or_field:
        if "." in provider_or_field:
            ...
            try:
                ...
                raise ValueError(
                    'No faker found for "{}({})"'.format(provider_or_field, args)
                )
