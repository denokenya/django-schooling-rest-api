...
class Factory:
    @classmethod
    def create(
        cls, locale=None, providers=None, generator=None, includes=None, **config
    ):
        if includes is None:
            includes = []
    ...
    def _get_provider_class(cls, provider, locale=""):
        ...
        msg = "Unable to find provider `{}` with locale `{}`".format(provider, locale)
    ...
    def _find_provider_class(cls, provider_path, locale=None):
        ...
        if getattr(provider_module, "localized", False):
            ...
            logger.debug(
                "Looking for locale `{}` in provider `{}`.".format(
            ...
            path = "{provider_path}.{locale}".format(
                provider_path=provider_path, locale=locale
            )
