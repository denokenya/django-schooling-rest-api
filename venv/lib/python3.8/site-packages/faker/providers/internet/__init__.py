...
class Provider(BaseProvider):
    ...
    def ascii_email(self):
        pattern = self.random_element(self.email_formats)
        return self._to_ascii("".join(self.generator.parse(pattern).split(" ")))
    ...
    def ascii_free_email(self):
        return self._to_ascii(self.user_name() + "@" + self.free_email_domain())

    @lowercase
    def ascii_company_email(self):
        return self._to_ascii(self.user_name() + "@" + self.domain_name())

    @slugify_unicode
    def user_name(self):
        pattern = self.random_element(self.user_name_formats)
        username = self._to_ascii(self.bothify(self.generator.parse(pattern)).lower())
        return username
    ...
    def hostname(self, levels=1):
        ...
        if levels < 1:
            return (
                self.random_element(self.hostname_prefixes) + "-" + self.numerify("##")
            )
        return (
            self.random_element(self.hostname_prefixes)
            + "-"
            + self.numerify("##")
            + "."
    ...
    def _get_all_networks_and_weights(self, address_class=None):
        ...
        all_networks = self._exclude_ipv4_networks(
            all_networks, _IPv4Constants._excluded_networks
        )
    ...
    def _get_private_networks_and_weights(self, address_class=None):
        ...
        private_networks = self._exclude_ipv4_networks(
            private_networks, _IPv4Constants._excluded_networks
        )
    ...
    def _random_ipv4_address_from_subnets(self, subnets, weights=None, network=False):
        ...
        try:
            subnet = choices_distribution(
                subnets, weights, random=self.generator.random, length=1
            )[0]
    ...
    def uri_path(self, deep=None):
        ...
        return "/".join(self.random_elements(self.uri_paths, length=deep))
