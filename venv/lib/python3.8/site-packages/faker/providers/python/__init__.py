...
class Provider(BaseProvider):
    ...
    def pyfloat(
        ...
        if positive and min_value is not None and min_value < 0:
            raise ValueError("Cannot combine positive=True and negative min_value")

        left_digits = (
            left_digits
            if left_digits is not None
            else (self.random_int(1, sys.float_info.dig))
        )
        right_digits = (
            right_digits
            if right_digits is not None
            else (self.random_int(0, sys.float_info.dig - left_digits))
        )
        sign = ""
        if (min_value is not None) or (max_value is not None):
            if max_value is not None and max_value < 0:
                max_value += (
                    1
                )  # as the random_int will be generated up to max_value - 1
            if min_value is not None and min_value < 0:
                min_value += 1  # as we then append digits after the left_number
            left_number = self._safe_random_int(min_value, max_value, positive)
        else:
            sign = "+" if positive else self.random_element(("+", "-"))
            left_number = self.random_number(left_digits)

        return float(
            "{}{}.{}".format(sign, left_number, self.random_number(right_digits))
        )
    ...
    def pydecimal(
        ...
        float_ = self.pyfloat(left_digits, right_digits, positive, min_value, max_value)
    ...
    def pytuple(
        ...
        *allowed_types
        ...
        return tuple(
            self.pyset(nb_elements, variable_nb_elements, value_types, *allowed_types)
        )
